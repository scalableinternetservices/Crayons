class ProfilesController < ApplicationController
  ...
  def photo_cache
    ...
    Rails.cache.fetch("#{cache_key}/user", expires_in: 60.min) do
      UserDevise.where( :user_devises_id => current_user_devise.id )
    end
  end

  def show
    @profile = UserDevise.find(params[:id])
    @photos = Photo.where( :user_devises_id => current_user_devise.id).paginate(:page => params[:page], :per_page => 5)
